...
private fun buildLinuxVlcCommand(
    ...
    if (!request.sourceAudioUrl.isNullOrBlank()) {
        return LinuxExternalPlayerCommandResult(null, "VLC does not support separate audio URLs")
    }
    val command = mutableListOf<String>().apply {
        ...
        add("--no-video-title")
        request.initialPositionMs.takeIf { it > 0L }?.let { ms ->
...
private fun buildLinuxKodiCommand(
    ...
    if (!request.sourceAudioUrl.isNullOrBlank()) {
        return LinuxExternalPlayerCommandResult(null, "Kodi does not support separate audio URLs")
    }
    if (request.initialPositionMs > 0L) {
