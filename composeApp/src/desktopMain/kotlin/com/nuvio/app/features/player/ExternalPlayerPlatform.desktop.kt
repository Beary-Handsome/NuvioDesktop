...
internal actual object ExternalPlayerPlatform {
    ...
    private val detectedWindowsPlayers: List<WindowsExternalPlayerInstall> by lazy {
    ...
    private val detectedLinuxPlayers: List<LinuxExternalPlayerInstall> by lazy {
        val players = detectLinuxExternalPlayers()
        DesktopRuntimeLog.info(
            "externalPlayer (Linux) detection complete count=${players.size} ids=${players.joinToString { it.id }}",
        )
        players
    }

    actual fun defaultPlayerId(): String? =
        if (isLinux) detectedLinuxPlayers.firstOrNull()?.id
        else detectedWindowsPlayers.firstOrNull()?.definition?.id

    actual fun availablePlayers(): List<ExternalPlayerApp> =
        if (isLinux) detectedLinuxPlayers.map { ExternalPlayerApp(id = it.id, name = it.name) }
        else detectedWindowsPlayers.map { install ->
            ExternalPlayerApp(
    ...
    actual fun open(
        ...
        DesktopRuntimeLog.info(
            "externalPlayer open requested platform=${if (isLinux) "linux" else "windows"} configuredId=${playerId ?: "none"} " +
        ...
        return if (isLinux) {
            openOnLinux(request, playerId)
        } else {
            openOnWindows(request, playerId)
        }
    }

    private fun openOnLinux(
        request: ExternalPlayerPlaybackRequest,
        playerId: String,
    ): ExternalPlayerOpenResult {
        val install = detectedLinuxPlayers.firstOrNull { it.id == playerId }
            ?: run {
                DesktopRuntimeLog.warn("External player unavailable id=$playerId")
                return ExternalPlayerOpenResult.NoPlayerAvailable
            }
        val commandResult = buildLinuxExternalPlayerCommand(install, request)
        val command = commandResult.command
            ?: run {
                DesktopRuntimeLog.warn("External player launch rejected id=$playerId reason=${commandResult.failureReason}")
                return ExternalPlayerOpenResult.Failed
            }
        return runCatching {
            DesktopRuntimeLog.info(
                "externalPlayer command prepared id=${install.id} executable=${install.executablePath} command=$command",
            )
            ...
            DesktopRuntimeLog.info(
                "externalPlayer launched id=${install.id} pid=${processPid ?: "unknown"} " +
                    "elapsedLaunchMs=${System.currentTimeMillis() - startMs} executable=${install.executablePath}",
            )
            DesktopExternalPlaybackWindowController.minimizeToTray(install.id, processPid)
            monitorExternalPlayerProcess(process, install.id, startMs)
            ExternalPlayerOpenResult.Opened
        }.getOrElse { throwable ->
            DesktopRuntimeLog.error("External player launch failed id=${install.id}", throwable)
            ExternalPlayerOpenResult.Failed
        }
    }

    private fun openOnWindows(
        request: ExternalPlayerPlaybackRequest,
        playerId: String,
    ): ExternalPlayerOpenResult {
        ...
        val install = detectedWindowsPlayers.firstOrNull { it.definition.id == playerId }
            ?: run {
